...
class NFW(object):
    ...
    def __init__(self, M200, z=0., CP=WMAP7(), c200=None):
        ...
        self.rho0 = np.power(self.CP.h, 2) * 278. * (200. / 3.) * np.power(self.c200, 3) / \
                    (np.log(1 + self.c200) - self.c200 / (1 + self.c200)) * U.solMass * U.kpc ** -3
        self.rmax = 2.1626 * self.Rs
        self.vmax = self.vc(self.rmax)
        return

    def vc(self, r):
        retval = np.power(4. * np.pi * C.G * self.rho0 * np.power(self.Rs, 3) / r * \
                        (np.log(1 + r / self.Rs) - 1. / (1 + self.Rs / r)), .5)
