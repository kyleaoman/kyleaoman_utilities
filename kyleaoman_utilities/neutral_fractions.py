...
def atomic_frac(
    ...
    """
    Computes particle atomic hydrogen fractions. See also molecular_frac and neutral_frac in this
    module.
    ...
    noCol:             The contribution of collisional ionisation to the overall ionisation rate is 
                       neglected.
    onlyCol:           The contribution of photoionisation to the overall ionisation rate is 
                       neglected.
    SSH_Thresh:        All particles above this density are assumed to be fully shielded, i.e. 
                       f_neutral=1.
    local:             Compute the local polytropic index.
    EAGLE_corrections: Determine which particles are on the EoS and adjust values accordingly.
    mu:                Mean molecular weight, default 1.22 (required with EAGLE_corrections).
    gamma:             Polytropic index, default 4/3 (required with EAGLE_corrections).
    fH:                Primordial hydrogen abundance, default 0.752 (required with EAGLE_corrections).
    T0:                EoS critical temperature, default 8000 K (required with EAGLE_corrections).
    
    Returns an array of the same shape as particle property inputs containing the atomic (HI) mass 
    fractions.
    """
...
def molecular_frac(
    ...
    To compute molecular mass of particle, multiply particle mass by hydrogen mass fraction and 
    molecular fraction.
    ...
    Returns an array of the same shape as particle property inputs containing the molecular mass 
    fractions.
...
def neutral_frac(
    ...
    """
    Computes particle neutral hydrogen fractions based on the fitting functions of Rahmati et al. 
    (2013a). By default, it uses the parameters of Table A1 (based on small cosmological volumes) for
    z > 1, and of Table A2 (based on a 50Mpc volume) for z < 1, to better account for the effects of 
    collisional ionisation on the self-shielding density. 
    
    To compute neutral (HI + H_2) mass of particle, multiply NeutralFraction by Hydrogen mass fraction
    and particle mass.
    ...
    local:             Compute the local polytropic index.
    EAGLE_corrections: Determine which particles are on the EoS and adjust values accordingly.
    SFR:               Particle star formation rates (required with EAGLE_corrections).
    mu:                Mean molecular weight, default 1.22 (required with EAGLE_corrections).
    gamma:             Polytropic index, default 4/3 (required with EAGLE_corrections).
    fH:                Primordial hydrogen abundance, default 0.752 (required with EAGLE_corrections).
    Habundance:        Particle Hydrogen mass fractions (required with EAGLE_corrections).
    T0:                EoS critical temperature, default 8000 K (required with EAGLE_corrections).
    rho:               Gas particle density (required with EAGLE_corrections).
    
    Returns an array of the same shape as particle property inputs containing the neutral mass 
    fractions.
    
    Original IDL function written by Rob Crain, Leiden, March 2014, with input from Ali Rahmati. Based
    on Rahmati et al. (2013). Translated from IDL to Python 2.7 by Kyle Oman, Victoria, December 2015,
