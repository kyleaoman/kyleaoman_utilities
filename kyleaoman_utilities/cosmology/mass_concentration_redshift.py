...
def _load_tables(CP=WMAP7()):
    try:
        sigma0_tab, M_tab_s = \
            np.genfromtxt(prefix + 'sigma0_' + CP.name + '.dat').T
        M_tab_c, c_tab = np.genfromtxt(prefix + 'Mc_z0_' + CP.name + '.dat').T
    except IOError:
        raise IOError('Missing tables, maybe they are unavailable for this'
                      ' cosmology (supported: WMAP7, Planck13).')
