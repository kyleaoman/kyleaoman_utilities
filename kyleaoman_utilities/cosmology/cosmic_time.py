...
def z_to_t(z_in, CP=WMAP7()):
    ...
    return np.power(CP.h * 100. * U.km * U.s**-1 * U.Mpc**-1, -1).to(U.Gyr) * np.power(3.0 * np.sqrt(1 - CP.Om), -1) * np.log((1 + theta) / (1 - theta))

def t_to_z(t_in, CP=WMAP7(), z_max=25.0):
    n_table = z_max * 100.
    z_table = np.linspace(z_max, 0.0, n_table)
    t_table = z_to_t(z_table, CP=CP)
    if np.min(t_table) > np.min(t_in):
        return t_to_z(t_in, CP=CP, z_max=z_max*10.)
